...
### arguments:
- `i::AbstractVector{Int}`: row indices
- `j::AbstractVector{Int}`: col indices
- `h::AbstractVector{<:Real}`: values s.t. h[1] = H[i[1], j[1]]
- `n::Int64`: dimension of hessian
### returns:
- `H::SparseMatrixCSC`: sparse, symmetric hessian
...
"""
function populate_hess_sparse(i::AbstractVector{Int},
...
...
"""
function populate_hess_sparse!(H::SparseMatrixCSC{Float64,Int},
...
- `i::AbstractVector{Int}`: row indices
- `j::AbstractVector{Int}`: col indices
- `jj::AbstractVector{<:Real}`: values s.t. jj[1] = J[i[1], j[1]]
...
"""
function populate_jac_sparse!(J::SparseMatrixCSC{Float64,Int},
                              i::AbstractVector{Int}, j::AbstractVector{Int},
                              jj::AbstractVector{<:Real})
...
### arguments:
    * `x::AbstractVector{<:Real}`: point to evaluate
    * `[model::JuMP.NLPEvaluator]`: (kwarg) initialized model to evaluate
...
"""
function f!(x::AbstractVector{<:Real}; model::JuMP.NLPEvaluator)
    return MOI.eval_objective(model, x)
end
...
### arguments:
    * `g::AbstractVector{<:Real}`: placeholder for gradient
    * `x::AbstractVector{<:Real}`: point to evaluate
    * `[model::JuMP.NLPEvaluator]`: (kwarg) initialized model to evaluate
...
"""
function g!(g::AbstractVector{<:Real}, x::AbstractVector{<:Real}; model::JuMP.NLPEvaluator)
    MOI.eval_objective_gradient(model, g, x)
end
...
...
"""
function H!(H::SparseMatrixCSC{Float64,Int}, x::AbstractVector{<:Real};
            model::JuMP.NLPEvaluator, structure::Int=0)
    m = JuMP.num_constraints(model; count_variable_in_set_constraints=false)
    ij_hess = MOI.hessian_lagrangian_structure(model)
    h = ones(length(ij_hess[1]))
    MOI.eval_hessian_lagrangian(model, h, x, 1.0, zeros(m))
...
...
"""
function h!(H::SparseMatrixCSC{Float64,Int}, x::AbstractVector{<:Real}; model::JuMP.NLPEvaluator,
            lam::AbstractVector{<:Real}=[1.0])
    ij_hess = MOI.hessian_lagrangian_structure(model)
    h = ones(length(ij_hess[1]))
    MOI.eval_hessian_lagrangian(model, h, x, 0.0, lam)
    populate_hess_sparse!(H, ij_hess[1], ij_hess[2], h)
end
...
...
"""
function j!(J::SparseMatrixCSC{Float64,Int64}, x::AbstractVector{<:Real}; model::JuMP.NLPEvaluator)
    m = JuMP.num_constraints(model; count_variable_in_set_constraints=false)
    n = JuMP.num_variables(model)
    @assert(size(J) == (m,n))
    ij_jac = MOI.jacobian_structure(model)
    j = ones(length(ij_jac[1]))
    MOI.eval_constraint_jacobian(model, j, x)
...
### arguments:
    * `c::AbstractVector{<:Real}`: placeholder for constraint evaluations
    * `x::AbstractVector{<:Real}`: point to evaluate
    * `[model::JuMP.NLPEvaluator]`: (kwarg) initialized model to evaluate
...
"""
function c!(c::AbstractVector{<:Real}, x::AbstractVector{<:Real}; model::JuMP.NLPEvaluator)
    # m = MathProgBase.numconstr(model.m)
    # con = fill(NaN, m)
    MOI.eval_constraint(model, c, x)
end
## =============================================================================